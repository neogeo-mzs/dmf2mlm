...
class ADPCMAEncoder:
	...
	def _jedi_table_init(self):
		...
		for step in range(49):
			for nib in range(16):
				...
				if (nib & 0x08) != 0:
					self.jedi_table[step * 16 + nib] = -value
				else:
					self.jedi_table[step * 16 + nib] = value

	def _ym2610_adpcma_decode(self, code: c_char) -> c_short:
		...
		return c_short(self.acc)

	def _ym2610_adpcma_encode(self, sample: c_short) -> c_char:
		tempstep: int
		code: c_char
		...
		if (self.diff >= self.tempstep):
			...
			self.diff -= tempstep
		tempstep >>= 1
		if (self.diff >= self.tempstep): code |= 1
	...
	def ym_encode(self):
		...
		if ((len(self.buffer) / 2) % 2 != 0):
			...
			self.in_buffer[len(self.in_buffer) - 1] = c_short(0x0000)
		else:
		...
		for i in range(0, len(self.buffer), 2):
			...
			self.in_buffer[i / 2] = c_short(self.in_buffer[i / 2])
		...
		for i in range(0, len(self.in_buffer), 2):
			...
			self.out_buffer[i / 2] = c_char(self.out_buffer[i / 2])